        ...
        for query_id in results:
            if len(results[query_id]) > top_k:
                for (doc_id, _) in sorted(results[query_id].items(), key=lambda item: item[1], reverse=True)[:top_k]:
                    ...
                    sentence_pairs.append([queries[query_id], corpus_text[0:256]])
...
#        print(sentence_pairs[0:3])
#        print(self.cross_encoder.predict(sentence_pairs[0:1500]))
        rerank_scores = [float(score) for score in self.cross_encoder.predict(sentence_pairs, batch_size=self.batch_size)[:,1]]
